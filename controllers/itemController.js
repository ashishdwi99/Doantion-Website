...
module.exports.myrequest_get = (req,res)=>{
    ...
    .select('_id name description address contactNumber date postedBy')
    .exec()
    .then(docs=>{
        const response = {
            items: docs.map(docs=>{
                return {
                    ...
                    contactNumber:docs.contactNumber,
                    date:docs.date,
...
module.exports.donatable = (req,res)=>{
    ...
    Item.find().select('name description address contactNumber date postedBy')
    .exec()
    .then(docs=>{
        const response = {
            items: docs.map(docs=>{
...
module.exports.deleteitem = (req,res)=>{
    ...
    .then((result)=>{
        Item.find({postedBy:req.query.postedBy})
        .select('name description address contactNumber date postedBy')
        .exec()
        .then(docs=>{
            const response = {
                items: docs.map(docs=>{
